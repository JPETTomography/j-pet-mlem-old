...
public:
  ...
  void init() {
    ...
    corrected_density_= new PixelGrid<F>(*tof_density_);
  ...
  void simulate_from_phantom(int n,  bool single=false) {
    ...
    *corrected_density_= *tof_density_;
    corrected_density_->divide_by(*acceptance_density_);
    F corrected_max=corrected_density_->max();
    //corrected_density_->divide_by(tof_max);
  ...
  const  F *corrected_density() {return corrected_density_->pixels();}
...
private:
  ...
  PixelGrid<F> *corrected_density_;

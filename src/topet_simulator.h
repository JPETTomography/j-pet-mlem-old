...
template <typename F> class TOPETSimulator {
 public:
  ...
  void init() {
    ...
    int height = 100;
    emitted_density_ =
        new PixelGrid<F>(point_t(-250, -250), point_t(250, 250), width, height);
    detected_density_ =
        new PixelGrid<F>(point_t(-250, -250), point_t(250, 250), width, height);
    tof_density_ =
        new PixelGrid<F>(point_t(-250, -250), point_t(250, 250), width, height);
  ...
  void simulate_from_phantom(int n) {
    ...
    int n_emitted_events =
        mc_->fill_with_events_from_phantom(emitted_events_.begin(),
                                           phantom_,
                                           fov_ll_x_,
                                           fov_ll_y_,
                                           fov_ur_x_,
  ...
  void simulate_from_single_point(int n) {
    ...
    int n_emitted_events = mc_->fill_with_events_from_single_point(
        emitted_events_.begin(), 0.0f, 0.0f, n);
    std::cerr << "emited " << n_emitted_events << " events" << std::endl;
    ...
    int n_detected_events =
        mc_->add_noise_to_detected(emitted_events_.begin(),

 ...
 public:
  ...
  int check_for_hits(RandomGenerator& gen,
    ...
    int step = ((n_detectors + inner - outer) % n_detectors >
                (n_detectors + outer - inner) % n_detectors) ? 1 :
        n_detectors - 1;
  ...
  short emit_event(RandomGenerator& gen,
    ...
    auto i_inner =
        c_inner.section(c_inner.angle(inner_secant.first), n_detectors);
    auto i_outer =
        c_outer.section(c_inner.angle(outer_secant.first), n_detectors);
    int detector1;

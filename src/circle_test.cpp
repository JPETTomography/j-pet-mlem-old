...
TEST_CASE("circle/secant", "circle secant") {
  ...
  SECTION("angle-90", "90 degrees from (0, 0)") {
    decltype(c) ::Event zero90(0., 0., M_PI_2);
  ...
  SECTION("angle-45", "45 degrees from (1, 0)") {
    decltype(c) ::Event xone45(1., 0., M_PI_4);
...
template <typename F> bool compare(F left, F right, F tol) {
...
TEST_CASE("circle/secant/math", "test using file generated by mathematica") {
  ...
  while (true) {
    ...
    line++;
    Circle<double> c(r);
    decltype(c) ::Event event(x, y, angle);

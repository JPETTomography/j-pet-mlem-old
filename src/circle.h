...
template <typename F = double> class circle {
 public:
  circle(F radius) : r(radius), r2(radius* radius) {}
  ...
  F angle(point_type p) { return std::atan2(p.y, p.x); }

  std::pair<angle_type, angle_type> secant_angles(event_type& e) {
    auto s = secant(e);
  ...
  std::pair<int, int> secant_sections(event_type& e, size_t n_detectors) {

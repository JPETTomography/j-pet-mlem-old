...
void ChangeSize(int w, int h) {
  ...
  p_matrix = glm::ortho(-height * aspect / 2.0f,
                        height * aspect / 2.0f,
                        -height / 2.0f,
                        height / 2.0f,
                        99.0f,
...
void SetupRC() {
  ...
  shader->link();
  mvp_matrix_uniform.add_shader(shader->shader());
  ...
  detector_view =
      new DetectorView<GLfloat>(geometry_plot, simulator.detector());
  phantom_view = new PhantomView(geometry_plot, simulator.phantom());
  event_view = new EventView<GLfloat>(geometry_plot,
                                      simulator.detector(),
                                      simulator.tof_begin(),
                                      simulator.tof_end(),

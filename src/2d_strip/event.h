...
template <typename F> struct ImageSpaceEventAngle {
  ImageSpaceEventAngle(F y_a, F z_a, F angle_a):y(y_a), z(z_a), angle(angle_a){};
  const F y;
  const F z;
  const F angle;
};
...
template<typename F> ImageSpaceEventTan<F> to_tan(const ImageSpaceEventAngle<F> &ea) {
  return  ImageSpaceEventTan<F>(ea.y, ea.z, tan(ea.angle));
}

template<typename F> ImageSpaceEventAngle<F> to_angle(const ImageSpaceEventTan<F> &ea) {

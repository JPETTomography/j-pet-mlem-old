...
class spet_reconstruction {
 ...
 public:
  ...
  void set_inverse_correlation_matrix() {
    ...
    inverse_correlation_matrix.resize(3, std::vector<T>(3, 0.0f));

    T pow_sigma_z = sigma_z * sigma_z;
    T pow_sigma_dl = sigma_dl * sigma_dl;
    T pow_gamma = gamma * gamma;

    std::cout << "pow_gammas: " << pow_sigma_z << " " << pow_sigma_dl << std::endl;

    inverse_correlation_matrix[0][0] = 1.0f/pow_sigma_z;//pow_gamma - (pow_sigma_z * pow_sigma_dl);
    inverse_correlation_matrix[0][1] = 0.0f;
    inverse_correlation_matrix[0][2] = 0.0f;

    std::cout <<"D1: " <<  1/pow_sigma_z << std::endl;

    inverse_correlation_matrix[1][0] = 0.0f;
    inverse_correlation_matrix[1][1] = 1.0f/pow_sigma_z;//pow_gamma - (pow_sigma_z * pow_sigma_dl);
    inverse_correlation_matrix[1][2] = 0.0f;

    std::cout <<"D2: " <<  1.0f/pow_sigma_z << std::endl;

    inverse_correlation_matrix[2][0] = 0.0f;
    inverse_correlation_matrix[2][1] = 0.0f;
    inverse_correlation_matrix[2][2] = 1.0f/pow_sigma_dl;// -pow_sigma_z * pow_sigma_z;

    std::cout <<"D3: " <<  1.0f/pow_sigma_dl << std::endl;

    inverse_correlation_matrix_factor =
        1.0f / ((pow_gamma * pow_sigma_z) + (pow_gamma * pow_sigma_z) +
  ...
  T kernel(T &y, T &tan, pixel_location &pixel) {
    ...
    T event_inv_cos = 1.0f / std::cos(angle * 180 * INVERSE_PI );
    T pow_event_inv_cos = event_inv_cos * event_inv_cos;


    std::cout << "INVERSE_CORRELATION_MATIRX: " << inverse_correlation_matrix[0][0] << " " << inverse_correlation_matrix[1][1] << " "
              << inverse_correlation_matrix[2][2] << std::endl;

    std::cout << "INVERSE_CORRELATION_MATIX_FACTOR: " << inverse_correlation_matrix_factor << std::endl;


    std::cout << "angle: " << std::atan(tan) << " " << angle * 180 * INVERSE_PI  << " inv_cos: " <<
            event_inv_cos << " pow_inv_cos: " << pow_event_inv_cos << std::endl;

    std::vector<T> vec_o(3, 0.0f);
    std::vector<T> vec_a(3, 0.0f);
    std::vector<T> vec_b(3, 0.0f);

    vec_o[0] = -(pixel.first + y - R_distance) * tan * pow_event_inv_cos;
    vec_o[1] = -(pixel.first + y + R_distance) * tan * pow_event_inv_cos;
    vec_o[2] = -(pixel.first + y) * event_inv_cos * (1.0f + (2.0f * tan * tan));
    ...
    vec_b[2] = -2.0f * pixel.first * event_inv_cos;
    ...
    std::cout << "element_aa: " << element_aa << " element_ob: " << element_ob << std::endl;

    T element_sum_aa_ob = element_aa + (2.0f * element_ob);

    T det_inverse_matrix = inverse_correlation_matrix[0][0] * inverse_correlation_matrix[1][1] *
                           inverse_correlation_matrix[2][2];

    std::cout << "DET MATRIX: " << det_inverse_matrix << std::endl;

    T element_before_exp = INVERSE_TWO_PI * (det_inverse_matrix / sqrt(element_sum_aa_ob));

    std::cout << "ELEMENT_BEFORE_EXP: " << element_before_exp << std::endl;

    T exp_element =
        -0.5f * (element_bb - ((element_ba * element_ba) / element_sum_aa_ob));
  ...
  void load_input(std::string fn) {
    ...
    for (;;) {
      ...
      std::cout << "LOCATIONS: " << "y: " << y << " z: " << z << " tan: " << tan <<std::endl;
  ...
  T sensitivity(T &y, T &z) {
    ...
    T a = (0.5f * Scentilator_length - z) / (R_distance - y);
    T b = (0.5f * Scentilator_length + z) / (R_distance + y);
    T c = (0.5f * Scentilator_length + z) / (R_distance - y);
  ...
  T event_tan(T &z_u, T &z_d) const { return (z_u - z_d) / (2.0f * R_distance); }
  ...
  pixel_location in_pixel(T &y, T &z) {
    ...
    return std::make_pair((((Scentilator_length/2.0f) + y) / pixel_size),
                          (((Scentilator_length/2.0f) + z) / pixel_size));
  }

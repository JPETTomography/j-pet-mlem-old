...
TEST_CASE("event conversions 1", "1") {
  StripDetector<double> detector(450.0, 200.0,
                                 200, 200,
                                   5.0, 5.0, 
                                   10, 63 );
...
ImageSpaceEventTan<double> img_tan =  to_tan(img_angle);
...
ImageSpaceEventAngle<double>  re_img_angle = detector.from_projection_space_angle(proj);

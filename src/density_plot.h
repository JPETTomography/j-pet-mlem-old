...
class DensityPlot {
 public:
  DensityPlot(GLuint width, GLuint height)
    ...
    mvp_matrix_uniform_.add_shader(shader_->shader());
  ...
  void set_mvp_matrix(glm::mat4 mvp) { mvp_ = mvp; }
  ...
  void set_pixmap(const GLfloat* p_bits) {
    ...
    glTexImage2D(
        GL_TEXTURE_2D, 0, GL_RED, width_, height_, 0, GL_RED, GL_FLOAT, p_bits);
  }

  void set_vertex(
      GLuint i, GLfloat x, GLfloat y, GLfloat z, GLfloat s, GLfloat t) {
    vertices_[5 * i] = x;
    vertices_[5 * i + 1] = y;
    vertices_[5 * i + 2] = z;
    vertices_[5 * i + 3] = s;
  ...
  void render() {
    ...
    for (int i = 0; i < 4; i++) {
      glVertexAttrib2f(
          GLT_ATTRIBUTE_TEXTURE0, vertices_[5 * i + 3], vertices_[5 * i + 4]);

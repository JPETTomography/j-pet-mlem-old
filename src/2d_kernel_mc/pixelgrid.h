...
public:
    ...
    PixelGrid(int n_row, int n_col, F pixel_width, F pixel_height)
        : n_row_(n_row),
          ...
          ll_y_(-n_col_*pixel_height_/2.0), values_(n_row_*n_col_) {
    }
    ...
    PixelGrid(int n_row, int n_col, F pixel_width, F pixel_height, F ll_z, F ll_y)
        : n_row_(n_row),
          ...
          pixel_height_(pixel_height),
          ll_z_(ll_z),
          ll_y_(ll_y), values_(n_row_*n_col_) {
    }
    ...
    int row(int y) {return static_cast<int>(floor( (y-ll_y_)/pixel_height_));};


    void add(F z, F y, F weight) {
        values_[index(col(z),row(y))]+=weight;
    }

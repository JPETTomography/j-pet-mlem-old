...
#pragma omp parallel for schedule(dynamic)
    ...
    for (ssize_t y = n_pixels_2 - 1; y >= 0; --y) {
      for (auto x = 0; x <= y; ++x) {
        ...
        if ((x*x + y*y) * s_pixel*s_pixel > detector_ring_.fov_radius()*detector_ring_.fov_radius()) continue;
        ...
        for (auto n = 0; n < n_mc_emissions; ++n) {
          ...
          lor_type lor;
          auto hits = detector_ring_.emit_event(l_gen, model, rx, ry, angle, lor);

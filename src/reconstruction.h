...
private:
  size_t fread(FILE *fin) {
    ...
    row_.resize(size);
    size_w+=::fread(&row_[0], sizeof(lor_t), row_.size(), fin);
    return size_w;
  }
...
void fill_pixel(double x, double y, double size_x, double size_y, int n_detectors, double *p, int n) {
  for(int i = 0; i<n; i++) {
...
template<typename P>
void fill_pixel(const P &pix, int n_detectors, double *p, int n) {
...
template<typename S, typename F>
  Row<Pixel<S, F>, F> *row_from_array(const Pixel<S, F> &pix, F *p, int n_detectors, int max_lors) {
